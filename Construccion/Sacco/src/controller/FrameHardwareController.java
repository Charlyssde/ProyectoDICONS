 ...
 * @author texch
 */
public class FrameHardwareController implements Initializable {
  ...
  @FXML
  private TableView<Hardware> tblHardware;
  @FXML
  ...
  private void buscarPorCriterio(MouseEvent event) throws SQLException {
    if (chbCriterio.getValue() == null || txtBuscar.getText().isEmpty()) {
      Mensaje.mostrarMensaje("Debe seleccionar un filtro y un criterio de busqueda");
  ...
  private void actualizarTabla() throws SQLException {
    tblHardware.getItems().clear();
    ObservableList<Hardware> visibleList = 
        (ObservableList<Hardware>) HardwareDAO.obtenerAllHardware();
    tblHardware.getItems().addAll(visibleList);
